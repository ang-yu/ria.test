...
theta <- function(train, valid, vars, params, learners, control) {
	...
	theta_y <- mlr3superlearner::mlr3superlearner(
		data = train$data[obs, na.omit(c(vars@D, vars@W, vars@M, vars@L, vars@Y))],
		target = vars@Y,
		library = learners,
	...
	if (length(params$natural) != 0) {
		...
		for (s in seq_along(params$natural)) {
			...
			theta2 <- mlr3superlearner::mlr3superlearner(
				data = add_psuedo(train$data[, na.omit(c(vars@D, vars@W, vars@L))], b3_train),
			...
			theta1 <- mlr3superlearner::mlr3superlearner(
				data = add_psuedo(train$data[, c(vars@D, vars@W)], b2_train),
				target = "tmp_ria_test_pseudo_y",
				library = learners,
				outcome_type = "continuous",
				folds = control$mlr3superlearner_folds,
	...
	for (s in seq_along(params$randomized)) {
		...
		theta3 <- mlr3superlearner::mlr3superlearner(
			data = add_psuedo(train$data[, c(vars@D, vars@W, vars@M)], b4_train),
			target = "tmp_ria_test_pseudo_y",
			library = learners,
			outcome_type = "continuous",
			folds = control$mlr3superlearner_folds,
		...
		theta2 <- mlr3superlearner::mlr3superlearner(
			data = add_psuedo(train$data[, c(vars@D, vars@W, vars@L)], b3_train),
			target = "tmp_ria_test_pseudo_y",
			library = learners,
			outcome_type = "continuous",
			folds = control$mlr3superlearner_folds,
